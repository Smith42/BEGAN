...
from utils.custom_ops import custom_fc, custom_conv1d
...
def began_discriminator(D_I, batch_size, num_filters, hidden_size,
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        layer_1 = tf.reshape(D_I, [-1, 800, 3])  # '-1' is batch size

        conv_0 = custom_conv1d(layer_1, 3, k_w=3, d_w=1, scope='ec0')
        conv_0 = tf.nn.elu(conv_0)

        conv_1 = custom_conv1d(conv_0, num_filters, k_w=3, d_w=1, scope='ec1')
        conv_1 = tf.nn.elu(conv_1)


        conv_2 = custom_conv1d(conv_1, num_filters, k_w=3, d_w=1, scope='ec2')
        conv_2 = tf.nn.elu(conv_2)


        layer_2 = custom_conv1d(conv_2, 2 * num_filters, k_w=3, d_w=2, scope='el2')
        layer_2 = tf.nn.elu(layer_2)

        conv_3 = custom_conv1d(layer_2, 2 * num_filters, k_w=3, d_w=1, scope='ec3')
        conv_3 = tf.nn.elu(conv_3)

        conv_4 =custom_conv1d(conv_3, 2 * num_filters, k_w=3, d_w=1, scope='ec4')
        conv_4 = tf.nn.elu(conv_4)


        layer_3 = custom_conv1d(conv_2, 3 * num_filters, k_w=3, d_w=2, scope='el3')
        layer_3 = tf.nn.elu(layer_3)

        conv_5 = custom_conv1d(layer_3, 3 * num_filters, k_w=3, d_w=1, scope='ec5')
        conv_5 = tf.nn.elu(conv_5)

        conv_6 = custom_conv1d(conv_5, 3 * num_filters, k_w=3, d_w=1, scope='ec6')
        conv_6 = tf.nn.elu(conv_6)


        layer_4 = custom_conv1d(conv_6, 4 * num_filters, k_w=3, d_w=2, scope='el4')
        layer_4 = tf.nn.elu(layer_4)

        conv_7 = custom_conv1d(layer_4, 4 * num_filters, k_w=3, d_w=1, scope='ec7')
        conv_7 = tf.nn.elu(conv_7)

        conv_8 = custom_conv1d(conv_7, 4 * num_filters, k_w=3, d_w=1, scope='ec8')
